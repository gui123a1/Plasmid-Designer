...
每条 read 给出 A/B/C 质量评级（Q20 比例/长度/N 碱基，ClinQC 口径），每处差异给出高/中/低置信度——替换变体从峰图提取峰级证据（突变峰占比 mutant_pct = 100×突变通道/(突变+参考通道)，信噪比 SNR = 突变通道峰高/全轨迹本底中位数，Mutation Surveyor 评分要素的工程实现），融合变异位 Q 值、支持 read 数与混合峰信号（次级峰占比 >30%）综合分级；单碱基插入额外计算插入峰强度比（插入峰面积/邻峰主峰中位数，≥0.6 视为峰真实存在——basecaller 在峰压缩区对 indel 系统性给低 Q，不再一票否决；<0.3 视为伪影），read 前导 ~20bp 峰形未稳定不产出插入峰证据，indel 单 read 低质量门槛更严（Q30）。indel 归一化为最左最简表示（Tan 2015，bcftools norm/GATK 同款），同聚物/重复区内不同 read 报告的等价 indel 合并印证、支持数不再分裂；tracy 可用时对每条 read 独立重新 basecall 并交叉比对，报出同一变体时打 corroborated_by_basecall 标记（仅提示两个 caller 读出一致，同一信号上错误相关、不改变置信度）。覆盖缺口按长度排序列出并提示从已测区边缘设计引物补测。
CDS 测序结论：特征边界偏差（手动标注常有 1–3bp）时自动对齐区间内最佳 ORF（起始密码子→终止密码子，长度优先）再翻译，被大 CDS 完全包含的小特征（如 6xHis 标签）不重复报告；结论按置信度分层——只将中/高置信变异（确证变异）重建进 CDS 判定，低置信变异（疑似测序噪声）不计入，单独在结论末尾给出待复核提示；翻译产物与参考做 BLOSUM62 全局比对，给出「完整覆盖 + 翻译产物一致 / 移码 / 无义提前终止 / 起始或终止密码子丢失 / 框内插入缺失 / 错义与同义」的整段判定，后果标注 Sequence Ontology 标准词表（stop_gained、frameshift_variant 等，与 VEP/snpEff/bcftools csq 对齐），结论文案为一句话因果链（变异位置 + 类型 + 后果 + 关键数字）；未覆盖区域按参考填充并明确标注未验证。
