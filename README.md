...
每条 read 给出 A/B/C 质量评级（Q20 比例/长度/N 碱基，ClinQC 口径），每处差异给出高/中/低置信度——替换变体从峰图提取峰级证据（突变峰占比 mutant_pct = 100×突变通道/(突变+参考通道)，信噪比 SNR = 突变通道峰高/全轨迹本底中位数，Mutation Surveyor 评分要素的工程实现），融合变异位 Q 值、支持 read 数与混合峰信号（次级峰占比 >30%）综合分级；indel 无对应单通道，按 Q 值 + 支持数分级且单 read 低质量 indel 门槛更严（Q30）。覆盖缺口按长度排序列出并提示从已测区边缘设计引物补测。
